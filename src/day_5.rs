...
pub mod sol{
    ...
    impl LineSegment{
        fn new(p1: Point, p2: Point) -> Self {
            ...
            else if ((p1.x as i64 - p2.x as i64) as i64).abs() == ((p1.y as i64 - p2.y as i64) as i64).abs() {
                if p1.x < p2.x{
                    return Self{a: p1, b: p2}
                }else{
                    return Self{a: p2, b: p1}
        ...
        fn is_diagonal(&self) -> bool {
            ((self.a.x as i64 - self.b.x as i64) as i64).abs() == ((self.a.y as i64 - self.b.y as i64) as i64).abs()
        }

        fn slope(&self) -> i8 {
            if self.a.y < self.b.y {1} else {-1}
        }

        fn integral_points(&self) -> Vec<Point> {
            if self.is_vertical() {
                if self.a.y < self.b.y {
                    return (self.a.y..=self.b.y).map(|y| Point::new(self.a.x, y)).collect()
                }else{
                    return (self.b.y..=self.a.y).map(|y| Point::new(self.a.x, y)).collect()
                }
            }else if self.is_horizontal() {
                if self.a.x < self.b.x {
                    return (self.a.x..=self.b.x).map(|x| Point::new(x, self.a.y)).collect()
                }else{
                    return (self.b.x..=self.a.x).map(|x| Point::new(x, self.a.y)).collect()
                }
            }else if self.is_diagonal(){
                if self.slope() > 0 {
                    return (self.a.x..=self.b.x).enumerate().map(|(i,x)| {
                        Point::new(x, self.a.y + i as u32)
                    }).collect()
                }else {
                    return (self.a.x..=self.b.x).enumerate().map(|(i,x)| {
                        Point::new(x, self.a.y - i as u32)
    ...
    impl Grid{
        fn new(size: u64) -> Self{
           let board = (0..size).fold(vec![], |mut row, _| {
                row.push((0..size).fold(vec![], |mut col, _| {col.push(0); col}));
                row
            });
            Self{
                size, board
    ...
    fn part1( content: String) -> u64 {
        ...
        let line_segments = content.split('\n').fold(vec![], |mut segments: Vec<LineSegment>, ls_data| {
            let mut points: Vec<Point> = vec![];
            for points_data in ls_data.split(" -> ").collect::<Vec<&str>>().iter(){
                let coordinates = points_data.split(',').map(|p| p.parse::<u32>().unwrap()).collect::<Vec<u32>>();
